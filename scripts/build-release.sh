...
# Publish Sigstore bundles; Kyverno uses the matching SigstoreBundle verifier.
cosign sign --new-bundle-format=true --use-signing-config=true --yes "$REF"
cosign attest --new-bundle-format=true --use-signing-config=true --yes --type cyclonedx --predicate evidence/sbom.cdx.json "$REF"
cosign attest --new-bundle-format=true --use-signing-config=true --yes --type slsaprovenance1 --predicate evidence/provenance.json "$REF"
cosign attest --new-bundle-format=true --use-signing-config=true --yes --type https://trusted-container-pipeline.dev/attestations/scan/v1 \
...
for variant in unsigned missing-provenance blocked-scan; do
  ...
  cosign sign --new-bundle-format=true --use-signing-config=true --yes "$variant_ref"
  trivy image --format cyclonedx --output "evidence/$variant-sbom.json" "$variant_ref"
  cosign attest --new-bundle-format=true --use-signing-config=true --yes --type cyclonedx --predicate "evidence/$variant-sbom.json" "$variant_ref"
...
PY
    cosign attest --new-bundle-format=true --use-signing-config=true --yes --type slsaprovenance1 --predicate evidence/blocked-provenance.json "$variant_ref"
    cosign attest --new-bundle-format=true --use-signing-config=true --yes --type https://trusted-container-pipeline.dev/attestations/scan/v1 \
